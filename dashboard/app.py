...
Interactive tool that queries a fitted Cox PH model for GEA survival risk.
Survival curves come from the actual fitted model's baseline_survival_function_,
...
def survival_probabilities_from_cox(cox_model, input_data, timepoints_days):
    """Return survival probability at each timepoint using the fitted Cox baseline.

    Uses lifelines' predict_survival_function which returns a DataFrame
    indexed by time in days with one column per row of input_data.
    """
    surv_df = cox_model.predict_survival_function(input_data)
    probs = {}
    # surv_df.index is the observed times in the training data
    times = surv_df.index.values
    for t in timepoints_days:
        # Find largest time <= t
        idx = np.searchsorted(times, t, side="right") - 1
        if idx < 0:
            probs[t] = 1.0
        else:
            probs[t] = float(surv_df.iloc[idx, 0])
    return probs
...
if st.sidebar.button("\U0001f52e Calculate Risk", key="predict_btn"):
    ...
    # ------------------------------------------------------------------
    # Survival probabilities from the ACTUAL fitted Cox model
    # (no hardcoded base_survival dict)
    # ------------------------------------------------------------------
    st.subheader("\U0001f4ca Estimated Overall Survival Probabilities")
    st.caption(
        "Survival curve is derived from the fitted Cox model's baseline survival function "
        "multiplied by this patient's partial hazard. Derived from TCGA-STAD; not validated for clinical use."
    )
    ...
    try:
        survival_probs = survival_probabilities_from_cox(cox_model, input_data, timepoints)
    except Exception as exc:  # noqa: BLE001
        st.warning(f"Could not compute survival curve from Cox model: {exc}")
    ...
    # Cohort comparison
    st.subheader("\U0001f4c8 Comparison to The Cancer Genome Atlas Cohort")
...
- Cox Proportional Hazards survival model (fitted via `scripts/python/survival_model.py`)
- Survival curves are computed from the fitted model's baseline survival function
