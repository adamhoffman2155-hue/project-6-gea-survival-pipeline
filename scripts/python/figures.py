...
def generate_km_curves(feature_matrix_csv: str, output_dir: str = "results/figures"):
    ...
    kmf.fit(
        msi_h["os_days"],
        msi_h["os_event"],
        label=f"Microsatellite Instability High (n={len(msi_h)})",
    )
    ...
    ax.set_title(
        "Kaplan-Meier Survival Curves by Microsatellite Instability Status",
        fontsize=15,
        fontweight="bold",
        pad=20,
    )
    ...
    plt.savefig(
        Path(output_dir) / "km_curves_msi.png", dpi=300, bbox_inches="tight", pad_inches=0.3
    )
    logger.info(f"Saved Kaplan-Meier curves to {Path(output_dir) / 'km_curves_msi.png'}")
    plt.close()


def generate_forest_plot(cox_model_pkl: str, output_dir: str = "results/figures"):
    ...
    for i, (_idx, row) in enumerate(summary.iterrows()):
        ax.plot([row["exp_ci_lower"], row["exp_ci_upper"]], [i, i], "b-", linewidth=2.5, zorder=2)

    ax.axvline(
        x=1, color="red", linestyle="--", linewidth=2.5, label="Hazard Ratio = 1 (No effect)"
    )
    ...
    ax.set_title(
        "Cox Proportional Hazards Model: Hazard Ratios", fontsize=15, fontweight="bold", pad=20
    )
    ...
    plt.tight_layout(pad=1.5)
    plt.savefig(
        Path(output_dir) / "forest_plot_cox.png", dpi=300, bbox_inches="tight", pad_inches=0.3
    )
    logger.info(f"Saved forest plot to {Path(output_dir) / 'forest_plot_cox.png'}")
    plt.close()


def generate_tmb_distribution(feature_matrix_csv: str, output_dir: str = "results/figures"):
    ...
    ax.violinplot(msi_data, positions=[0, 1], showmeans=True, showmedians=True)

    ax.set_xticks([0, 1])
    ax.set_xticklabels(msi_labels, fontsize=12)
    ax.set_ylabel(
        "Tumor Mutational Burden (mutations per megabase)", fontsize=13, fontweight="bold"
    )
    ax.set_title(
        "Tumor Mutational Burden Distribution by Microsatellite Instability Status",
        fontsize=15,
        fontweight="bold",
        pad=20,
    )
    ...
    plt.tight_layout(pad=1.5)
    plt.savefig(
        Path(output_dir) / "tmb_distribution.png", dpi=300, bbox_inches="tight", pad_inches=0.3
    )
    logger.info(
        f"Saved Tumor Mutational Burden distribution to {Path(output_dir) / 'tmb_distribution.png'}"
    )
    plt.close()


def generate_ddr_burden_plot(feature_matrix_csv: str, output_dir: str = "results/figures"):
    ...
    ax.set_title(
        "DNA Damage Repair Gene Mutation Burden Distribution",
        fontsize=15,
        fontweight="bold",
        pad=20,
    )
    ...
    plt.tight_layout(pad=1.5)
    plt.savefig(
        Path(output_dir) / "ddr_burden_distribution.png",
        dpi=300,
        bbox_inches="tight",
        pad_inches=0.3,
    )
    logger.info(
        f"Saved DNA Damage Repair burden plot to {Path(output_dir) / 'ddr_burden_distribution.png'}"
    )
    plt.close()
